...
ordersRouter.get("/mine", requireAuth, (req, res) => {
  const rows = getOrdersByUserId(req.user!.id);
...
ordersRouter.get("/by-session/:sessionId", (req, res) => {
  const row = getOrderBySessionId(req.params.sessionId);
